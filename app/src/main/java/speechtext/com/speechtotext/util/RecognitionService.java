...
public class RecognitionService {
    ...
    private static final String TAG = RecognitionService.class.getName();
...
//    private PartialResultsListener partialResultsListener;
    ...
    public RecognitionService(final Context pcontext) {
        context = pcontext;
...
//                this.requestExtras = requestExtras;
                ...
                if (!checkPermissions()) {
                    final Exception aiError = new Exception("RECORD_AUDIO permission is denied. Please request permission from user.");
                    onError(aiError);
                    return;
                }
...
//    }
    ...
    private class InternalRecognitionListener implements RecognitionListener {
        ...
        public void onError(final int error) {
            ...
            if (recognitionActive) {
                final Exception aiError;

                if (errorMessages.containsKey(error)) {
                    final String description = errorMessages.get(error);
                    aiError = new Exception("Speech recognition engine error: " + description);
                } else {
                    aiError = new Exception("Speech recognition engine error: " + error);
                }

                RecognitionService.this.onError(aiError);
            }
            stopInternal();
        }
        ...
        public void onResults(final Bundle results) {
            if (recognitionActive) {
                ...
                if (recognitionResults == null || recognitionResults.isEmpty()) {
                    // empty response
                    RecognitionService.this.onResult("");
                } else {
...
//                            if (response != null) {
                    RecognitionService.this.onResult(recognitionResults.get(0).toUpperCase());
//                        } else {
...
//                    GoogleRecognitionServiceImpl.this.onPartialResults(partialRecognitionResults);
    ...
    public void setListener(final SpeechListener listener) {
    ...
    protected void onResult(final String response) {
        if (listener != null) {
